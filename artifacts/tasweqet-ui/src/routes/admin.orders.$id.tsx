...
function AdminOrderDetail() {
  ...
  return <DashboardShell brand="طلبات بيتك" role="سوبر أدمن" nav={adminNav} title={data ? `الطلب ${data.code}` : "تفاصيل الطلب"}>
    ...
    <div className="flex flex-col gap-md">
      ...
      <Card className="p-md"><SectionTitle title="المطعم والتوصيل" icon="storefront" /><p>{data.restaurant.name} {data.branchName ? `— ${data.branchName}` : ""}</p><p className="text-on-surface-variant">{data.driver ? `المندوب: ${data.driver.name} · ${data.driver.phone || ""} · ${data.driver.area}` : data.dispatchStatus.state === "actively_offered" ? "يوجد عرض توصيل نشط بانتظار الرد" : data.dispatchStatus.state === "retry_scheduled" ? "لا يوجد مندوب مؤهل الآن؛ إعادة المحاولة مجدولة" : data.dispatchStatus.state === "terminal" ? "انتهت مرحلة الإسناد" : "لم يتم تعيين مندوب"}</p></Card>
